...
function App() {
  ...
  The toggle for Focus hour */}
        ...
        <div className='flex justify-center items-center mt-[2vh]'>
          ...
          <label htmlFor="toggle" className="cursor-pointer align-middle">
            <div className="relative">
              ...
              {/* Thumb */}
              <div
                className={`${isChecked ? 'translate-x-6 bg-pink-500' : 'translate-x-0 bg-white'
        ...
        <div className='w-full flex justify-center items-center mt-[2vh]'>
          {/* <input type="text" name="" id="" onChange={(e) => { setNewWebsiteName(e.target.value) }} />
          <button onClick={() => {
            addWebsitesToBlock(newWebsiteName)
          }}>Add</button> */}
          <button className='text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-xs px-5 py-2.5 text-center me-2 mb-2' onClick={() => {
            setShowWebsiteEditPage(true)
          }}>Edit websites to block</button>
        </div>
        ...
        <div className='absolute bottom-0 flex text-center mx-auto px-auto mb-[5vh] justify-center items-center flex-col'>
          <p>Focus mode ON means, blocking distracting websites. <a target="_blank" rel="noreferrer" href='https://github.com/pritipsingh/The-Productive-Champion'><span> Click here</span> to read about which websites are blocked or to add more websites.</a></p>
          <a target="_blank" rel="noreferrer" href='https://github.com/pritipsingh/The-Productive-Champion' className='mt-[2vh] underline'>Leave a ⭐️ on GitHub</a>
        </div>
      </div>
    );
  }
